...
|---|---|
| 제품 표기 | `영점손해사정` |
| 시각 테마 | 기본 `네온사인`은 기존 화면을 유지하고, `스토리형 괴담 처리`를 선택형 보조 테마로 제공 |
...
| 플랫폼 | 데스크톱 웹, 키보드 우선 |
| 입력 | WASD/방향키 이동, SHIFT 달리기, SPACE 복구, M 음소거 |
| 언어 | 한국어 |
...
```text
HG-01 권장안대로 승인. V2.1 정사, 제품명 영점손해사정,
...
`settings.theme`은 화면 연출과 초상화만 바꾸는 프로필 설정이다. `neon`은 기존 네온사인 방향을 유지하고, `casefile`은 첫 디자인의 초상화와 스토리형 괴담 처리 연출을 사용한다. 테마는 전투 규칙·스토리 정사·진행 자격을 바꾸지 않는다.
...
- 저장 키: `zero-loss-adjustment.profile`
- 이전 저장 키 `neon-breach.profile`은 최초 실행 시 새 키로 복사하고 원본도 보존한다.
- 현재 스키마: `1`
...
- 손상 JSON:
  - 원문을 `zero-loss-adjustment.profile.corrupt`에 한 번 백업
  - 경고 기록 후 기본 프로필로 복구
- 마이그레이션:
...
- 완료·seen·archive·통신 모드·`settings.muted`·`settings.theme` 직렬화
